...
local_t ** crearCC( int * pisos, int * localesxPiso ){
   ...
   scanf( " %d", localesxPiso );
   centroComercial = malloc( * localesxPiso * sizeof( local_t * ) );
   for( i = 0; i < * pisos; i++ ){
...
void generarRegistroDePagos( local_t ** centroComercial, int pisos, int localesxPiso ){
   FILE * registro;
   int i, j, costoMetroCuadrado;
   registro = fopen( "registro.txt", "w" );
   printf( "Ingrese el costo del metro cuadrado (redondeado al entero mas cercano) a la fecha: " );
   scanf( "%d", &costoMetroCuadrado );
   for( i = 0; i < pisos; i++ ){
      for( j = 0; j < localesxPiso; j++ ){
         if( centroComercial[ i ][ j ].idLocal != -1 ){
